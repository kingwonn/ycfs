...
- 翻译:核实 X-CUBE-MCSDK 是否覆盖单相 100k+ RPM 拓扑;调研 ST/Infineon/TI 单相高速参考设计;产出带厂商文档溯源的路线建议,为 D-001 补吹风机线限定语(修订案交人签字)。
- 依赖:无(若立法者有现成单相方案资料,直接提供可跳过调研)
- 验收(机器可查):
  - 调研报告落 docs/,每个结论带厂商文档溯源(URL/文档号/页)
  - 若确认 MCSDK 不覆盖:吹风机线驱动路线选项 ≥2 个带利弊与工具链影响
  - D-001 修订附注经人签字后更新 DECISIONS.md
- 证伪/回退:若核实 MCSDK 实际覆盖单相高速 → 附注撤销,零成本;若不覆盖而未核实就开工 → 吹风机线 CI/领域包返工,成本高。承重卡。
...
- 翻译:已知答案的 RFP→方案题、埋已知 bug 的固件修复题、规格书 QA 题——agent 的出厂检验,也是 L4 覆盖增长的校准基线。
- 依赖:无(题目真值可先从公开规格书构造)
- 验收(机器可查):
  - 基准 ≥N 题,每题真值带出生证(出处/谁定/何时)
  - 跑分脚本输出通过率;**题目真值非被测 agent 自产**(字节同源探针)
- 证伪/回退:承重卡——没有它,「agent 变好了」永远是感觉而非数字。
