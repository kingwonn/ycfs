...
- 状态:`blocked-on-human`(**骨架已落地 R18,D-002 授权先行**;表形态校准仍等 Q2 样例。docs/iec60730-coverage.md 12 组件×POST/BIST×真值锚;selftest.c 表驱动调度,NOT_IMPL 显式上报绝不伪 PASS(连 CRC 都拒绝对空缓冲伪自检);CRC32 纯函数已知答案向量验证;gate 第 13 腿:表↔代码 ID 一一对应+真值锚非空+IMPL 棘轮)
- 原话:"可以完成一些表格自测"
...
- 状态:`done`(第 R16 轮策略内核;**执行点接线随 A1 适配层**——opencode plugin permission.ask/PreToolUse 等价钩子届时调用本内核裁决。governance/data_gate.py:NDA 工件外发无具名确认即硬停;**概括授权("*"/空指向)结构性无效;approval 消耗式**(一次一件一放行,同件二次外发需再确认);非 NDA 走 D-003 云默认;restricted 项目对外部目的地**无放行边**(有审批也不行);未知目的地即拒。单测 10/10,gate 第 11 腿 data-gate。)
- 原话:"Q3,数据都可以,先追求SOTA效果"
- 翻译:策略内核=云默认+NDA 具名确认+可收紧路由;执行点随 A1。
- 验收记录:gate 11/11 绿(data-gate asserts=10);三条卡面验收全过(硬停/概括无效/路由与 D-003 一致)。
...
### B5 · MISRA 偏差登记册(+静态分析腿激活)
- 状态:`done`(第 R17 轮。**static-analysis 腿激活**:plain cppcheck 全 severity 零发现(修 constVariablePointer;comparePointers 内联抑制→DEV-001);MISRA 经 dump 两步法(2.13 的 --addon 集成静默失效,探针验证后绕行),基线 57→**49**(修 12.1×8/12.3×5/8.4×3),棘轮上限锁 49 只降不升;docs/misra-deviations.md 登记 9 条偏差(规则级,行级一一对应随认证阶段商业工具,见 D-004);**未登记规则违规即红**(负向自测:清空登记册→49 条全触红)。批准人签字挂 Q11。修复连带动了 bench 真值文件→哈希锁如设计触红→重签复核绿。)
- 原话:首席工程师 review C5
- 翻译:静态分析腿 + 规则级偏差登记册 + 棘轮。
- 验收记录:gate 12/12 绿(plain=0, misra=49≤49);负向自测过。
...
- 状态:`done`(第 R14 轮。dvpr/ 落地:吹风机种子 DVP&R 15 行(电气/热-温控/声学/电机性能/EMC-闪变/法规能效/安规,每行 limit+**出处**+method+stage+证据槽+签字槽,limit 全部引用已核实来源或诚实标 PLACEHOLDER)+ DFMEA 6 条(S/O/D/RPN);check_dvpr.py 机器校验:出处非空、**高 RPN(≥100) 必有验证链接**、链接双向存在、**L-real 行有证据必须有签字(C1 语义)**、行数下限;gate 第 10 腿 dvpr-check。负向自测 3/3(高 RPN 裸奔/有证据无签字/出处掏空均触红)。)
- 原话:首席工程师 review A2/B3
- 翻译:DVP&R 骨架 + DFMEA→验证推导链,机器查完整性;真机签字对行签。
- 验收记录:gate 10/10 绿(rows=15, dfmea=6);负向自测 3/3。
...
- 状态:`done`(第 R14 轮,随 I1 落地:DV-A01 声功率 ≤77dB(A)/DV-A02 主音调移出可听域(音质行)/DV-R01 待机 <0.5W(ErP Lot 6, EC 1275/2008)/DV-C02 闪变(IEC 61000-3-3),限值全部带法规编号或对标出处,check_dvpr 出处非空校验守护。)
- 原话:首席工程师 review B4/C4
- 翻译:声学与 ErP 行并入 DVP&R,带法规出处。
- 验收记录:见 I1(gate dvpr-check 腿)。
...
- 状态:`done`(第 R16 轮机器验收部分;**占位设计评审签字仍在 PENDING_HUMAN**。docs/ota-partition-plan.md:BOOT 32K/APP 472K@0x08008000/PARAMS 4K 分区表,双 bank OTA 路线(DBANK 细节标 PLACEHOLDER 待读 RM0440)、回滚语义=新镜像活过看门狗才算(被测不能自证)、RDP L1 起步 L2 过方向门禁;链接脚本落分区并构建绿;gate cross-compile 腿增分区存在性断言,FLASH 预算 512K→472K 收紧。)
- 原话:首席工程师 review C3
- 翻译:先锁分区防返工;OTA/签名实现后置。
- 验收记录:gate 11/11 绿(分区断言过,text=1660B 链接于 APP 分区);人签字项挂 PENDING_HUMAN。
